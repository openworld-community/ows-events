...
class EventsStateController {
	...
	constructor() {
		this.events = JSON.parse(fs.readFileSync('./assets/db/events.json', 'utf-8')) || [];
	...
	getEvents(query?: FindEventParams | undefined): EventOnPoster[] {
		...
		const autoGeneratedEventsCount = 10 - events.length;

		return [...events, ...this.getAutogenEvents({ autoGeneratedEventsCount, ...query })].sort(
			(a, b) => b.date - a.date
		);
	}
	...
	getAutogenEvents({
		...
		const autoGeneratedEvents: EventOnPoster[] = this.autogeneratedEvents.filter((e) => {
			if (country && city) {
		...
		const newAutoGeneratedEventsCount = autoGeneratedEventsCount - autoGeneratedEvents.length;
		...
		this.autogeneratedEvents = [...this.autogeneratedEvents, ...autoGeneratedEvents];
	...
	findEvents(query: FindEventParams) {
		...
		if (query.searchLine) {
			...
			events = events.filter((event) =>
				[event.title, event.description, event.location.country, event.location.city]
					.join(' ')
					.toLowerCase()
